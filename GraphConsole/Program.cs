...
namespace GraphConsole
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            do
            {
                ConsoleX.WriteLine("Введите граф (0 - список смежности, 1 - список дуг, 2 - выход)",
                    ConsoleColor.White);
                var i = ConsoleX.GetValue("Тип ввода", 1);
                ...
                if (i == 0)
                {
                    ...
                    for (var j = 0; j < nv; j++)
                    {
                        ...
                        for (var k = 0; k < nv2; k++) dict[j].Add(ConsoleX.GetValue<int>($"{k} вершина"));
                    }
                ...
                for (var j = 0; j < facets.Count; j++)
                {
                    ...
                    foreach (var item in facets[j]) ConsoleX.Write($"{item} ", ConsoleColor.White);
